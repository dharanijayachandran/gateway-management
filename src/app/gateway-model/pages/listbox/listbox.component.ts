...
export class ListboxComponent implements DoCheck, OnChanges {
  ...
  static DEFAULT_FORMAT = {
    ...
    all: 'Select All',
    none: 'Deselect All',
    direction: ListboxComponent.LTR,
    draggable: true,
    locale: undefined
  };
  ...
  @Input() height = '100px';
  @Input() filter = false;
  @Input() format = ListboxComponent.DEFAULT_FORMAT;
  @Input() sort = false;
  ...
  constructor(private differs: IterableDiffers) {
    this.available = new BasicList(ListboxComponent.AVAILABLE_LIST_NAME);
    this.confirmed = new BasicList(ListboxComponent.CONFIRMED_LIST_NAME);
  }
  ...
  ngOnChanges(changeRecord: { [key: string]: SimpleChange }) {
    ...
    if (changeRecord['format']) {
      ...
      if (typeof (this.format.direction) === 'undefined') {
        this.format.direction = ListboxComponent.LTR;
      }

      if (typeof (this.format.add) === 'undefined') {
        this.format.add = ListboxComponent.DEFAULT_FORMAT.add;
      }

      if (typeof (this.format.remove) === 'undefined') {
        this.format.remove = ListboxComponent.DEFAULT_FORMAT.remove;
      }

      if (typeof (this.format.all) === 'undefined') {
        this.format.all = ListboxComponent.DEFAULT_FORMAT.all;
      }

      if (typeof (this.format.none) === 'undefined') {
        this.format.none = ListboxComponent.DEFAULT_FORMAT.none;
      }

      if (typeof (this.format.draggable) === 'undefined') {
        this.format.draggable = ListboxComponent.DEFAULT_FORMAT.draggable;
      }
    }
  ...
  direction() {
    return this.format.direction === ListboxComponent.LTR;
  }
